			...
			<view :style='{"minHeight":"100vh","alignContent":"flex-start","padding":"20rpx 0 240rpx 0","alignItems":"flex-start","flexWrap":"wrap","background":"#ffffff","display":"block","width":"100%","position":"relative","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"24rpx 24rpx 20rpx 24rpx","background":"#ffffff","display":"flex","height":"auto","order":"3"}'>
					<view  :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' class="search-form round">
						...
						<input placeholder-style="color: #999" :style='{"border":"2rpx solid #dddddd","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333333","borderRadius":"60rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.yuyuebianhao" type="text" placeholder="预约编号" ></input>
					</view>
					...
					<view class="list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"4"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"padding":"0 0 20rpx 0","boxShadow":"0 2rpx 12rpx #cccccc","margin":"0 0 30rpx 0","borderColor":"#eeeeee","borderRadius":"12rpx","alignItems":"flex-start","flexWrap":"wrap","borderWidth":"0 0 0px 0","display":"flex","width":"48%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"width":"100%","padding":"0 20rpx 0 20rpx","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
								...
								<view class="order-meta">状态：{{product.fuwuzhuangtai || '待服务'}} / {{product.sfsh=='是'?'已接单':'待接单'}}</view>
								<view class="order-meta" v-if="product.yuyueriqi">上门：{{product.yuyueriqi}} {{product.yuyueshiduan}}</view>
							...
							<view v-if="role=='服务人员' && product.sfsh!='是'" class="accept-btn" @tap.stop="acceptOrder(product)">接单</view>
							<!-- #ifdef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwudingdan','修改')) || (!userid && isAuthFront('fuwudingdan','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwudingdan','删除')) || (!userid && isAuthFront('fuwudingdan','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwudingdan','修改')) || (!userid && isAuthFront('fuwudingdan','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#ff6f9f","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#ff6f9f","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwudingdan','删除')) || (!userid && isAuthFront('fuwudingdan','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#ff0000","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#ff0000","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view  :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"auto","padding":"0","fontSize":"24rpx","whiteSpace":"nowrap"}'>项目编号</view>
						<input :style='{"border":"0","width":"80%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","background":"#fff","height":"60rpx"}' placeholder="请输入项目编号" v-model="searchForm.xiangmubianhao">
					</view>
					<view  :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
	...
	export default {
		...
		async onShow() {
			if(uni.getStorageSync("useridTag")==1){
				...
				remindRes = await this.$api.getPublic(urlfuwuzhuangtai)
				if(remindRes.count>0) {
					var name = remindRes.data.join(',')
					await uni.showModal({
						title: `${remindRes.count}条服务状态预警`,
						content: `待服务订单，包含：${name}`,
						showCancel: false
					})
				}
			} else {
		...
		async onLoad(options) {
			this.acceptMode = options && options.acceptMode == 1
			this.hasNext = true
			// 重新加载数据
			// if (this.mescroll) this.mescroll.resetUpScroll()
		},
		...
		methods: {
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await that.$api.del('fuwudingdan', JSON.stringify([id]));
							that.$utils.msg('删除成功');
							that.hasNext = true
							// 重置分页参数页数为1
							that.search()
			...
			acceptOrder(row) {
				uni.showModal({
					title: '接单确认',
					content: '确认接下该家政服务订单吗？',
					success: async (res) => {
						if(!res.confirm) return
						let detailRes = await this.$api.info('fuwudingdan', row.id)
						let order = detailRes.data || row
						order.sfsh = '是'
						order.shhf = order.shhf || '服务人员已接单，请按预约时间上门服务。'
						order.fuwuzhuangtai = order.fuwuzhuangtai || '待服务'
						order.jiedanshijian = this.$utils.getCurDateTime()
						await this.$api.update('fuwudingdan', order)
						this.$utils.msg('接单成功，请及时联系用户确认上门细节')
						this.search()
					}
				})
			},
...
<style lang="scss" scoped>
	.content {
		...
		background: linear-gradient(180deg, #fff1f5 0%, #ffe6ee 100%);
	}
	.order-meta {
		width: 100%;
		color: #9b6074;
	...
	.accept-btn {
		...
		color: #fff;
		font-size: 28rpx;
		background: linear-gradient(135deg, #ff6f9f, #ff9ab9);
		box-shadow: 0 10rpx 22rpx rgba(255, 111, 159, .22);
	}
	.category-one .tab {
		cursor: pointer;
		padding: 0;
