			...
			<view :style='{"minHeight":"100vh","alignContent":"flex-start","padding":"20rpx 0 240rpx 0","alignItems":"flex-start","flexWrap":"wrap","background":"#ffffff","display":"block","width":"100%","position":"relative","height":"auto"}'>
				...
				<view :style='{"alignContent":"flex-start","alignItems":"flex-start","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","position":"initial","height":"auto","order":"4"}'>
					...
					<view class="list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"4"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"padding":"0 0 20rpx 0","boxShadow":"0 2rpx 12rpx #cccccc","margin":"0 0 30rpx 0","borderColor":"#eeeeee","borderRadius":"12rpx","alignItems":"flex-start","flexWrap":"wrap","borderWidth":"0 0 0px 0","display":"flex","width":"48%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
							<image :style='{"width":"100%","objectFit":"cover","borderRadius":"12rpx 12rpx 0 0","display":"block","height":"320rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.fuwutupian)" :src="product.fuwutupian.split(',')[0]"></image>
							<image :style='{"width":"100%","objectFit":"cover","borderRadius":"12rpx 12rpx 0 0","display":"block","height":"320rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.fuwutupian?baseUrl+product.fuwutupian.split(',')[0]:''"></image>
							...
							<view class="listmr" :style='{"width":"100%","padding":"0 20rpx 0 20rpx","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
								...
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"1.8","fontSize":"28rpx","textOverflow":"ellipsis","fontWeight":"500"}'>服务类型:{{product.fuwuleixing}}</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view  :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"auto","padding":"0","fontSize":"24rpx","whiteSpace":"nowrap"}'>员工姓名</view>
						<input :style='{"border":"0","width":"80%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","background":"#fff","height":"60rpx"}' placeholder="请输入员工姓名" v-model="searchForm.yuangongxingming">
					</view>
					<view  :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"auto","padding":"0","fontSize":"24rpx","whiteSpace":"nowrap"}'>服务类型</view>
						<input :style='{"border":"0","width":"80%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","background":"#fff","height":"60rpx"}' placeholder="请输入服务类型" v-model="searchForm.fuwuleixing">
					</view>
					<view  :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>服务确认</view>
						<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
							<view @click="fuwuquerenChange(item)" v-for="(item,index) in fuwuquerenOptions" :key="index" class="screenTab" :class="searchForm.fuwuqueren==item?'screenTabActive':''">{{item}}</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await that.$api.del('fuwujilu', JSON.stringify([id]));
							that.$utils.msg('删除成功');
							that.hasNext = true
							// 重置分页参数页数为1
							that.search()
			...
			async search(){
				...
				if(this.searchForm.fuwuqueren){
					searchForm['fuwuqueren'] = '%' + this.searchForm.fuwuqueren + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
				let res = {};
				if(this.userid) {
