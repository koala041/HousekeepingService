					...
					<view class="list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"4"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"padding":"0 0 20rpx 0","boxShadow":"0 2rpx 12rpx #cccccc","margin":"0 0 30rpx 0","borderColor":"#eeeeee","borderRadius":"12rpx","alignItems":"flex-start","flexWrap":"wrap","borderWidth":"0 0 0px 0","display":"flex","width":"48%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"width":"100%","padding":"0 20rpx 0 20rpx","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"1.8","fontSize":"28rpx","textOverflow":"ellipsis","fontWeight":"500"}'>{{product.yuyuebianhao}}</view>
								...
								<view :style='{"padding":"0 10rpx 0 0","order":"4"}'>
									...
									<text :style='{"color":"#999999","lineHeight":"50rpx","fontSize":"24rpx"}'>{{product.yonghuzhanghao}}</text>
								</view>
							</view>
							<!-- #ifdef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwuyuyue','修改')) || (!userid && isAuthFront('fuwuyuyue','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwuyuyue','删除')) || (!userid && isAuthFront('fuwuyuyue','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwuyuyue','修改')) || (!userid && isAuthFront('fuwuyuyue','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#ff6f9f","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#ff6f9f","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwuyuyue','删除')) || (!userid && isAuthFront('fuwuyuyue','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#ff0000","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#ff0000","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		async onShow() {
			if(uni.getStorageSync("useridTag")==1){
				...
				remindRes = await this.$api.getPublic(urldingdanzhuangtai)
				if(remindRes.count>0) {
					var name = remindRes.data.join(',')
					await uni.showModal({
						title: `${remindRes.count}条订单状态预警`,
						content: `服务已预约，包含：${name}`,
						showCancel: false
					})
				}
			} else {
				this.userid = "";
				if(
					this.isAuthFront('fuwuyuyue','预约金额统计')
					||this.isAuthFront('fuwuyuyue','预约服务类型统计')
				) {
					this.staticType = true
				}else {
					this.staticType = false
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`fuwuyuyue`, params);
                } else {
                    res = await this.$api.list(`fuwuyuyue`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await that.$api.del('fuwuyuyue', JSON.stringify([id]));
							that.$utils.msg('删除成功');
							that.hasNext = true
							// 重置分页参数页数为1
							that.search()
			...
			async search(){
				...
				let res = {};
				if(this.userid) {
					res = await this.$api.page(`fuwuyuyue`, searchForm);
				} else {
					res = await this.$api.list(`fuwuyuyue`, searchForm);
				}
				// 如果是第一页数据置空
