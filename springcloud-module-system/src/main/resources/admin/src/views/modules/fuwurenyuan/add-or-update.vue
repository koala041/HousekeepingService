	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="服务技能" prop="fuwujineng" >
					<el-input v-model="ruleForm.fuwujineng" placeholder="服务技能" clearable  :readonly="ro.fuwujineng"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="服务技能" prop="fuwujineng" >
					<el-input v-model="ruleForm.fuwujineng" placeholder="服务技能" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="fileupload" v-if="type!='info'&& !ro.zizhizhengshu" label="资质证书" prop="zizhizhengshu" >
					<file-upload
						tip="点击上传资质证书"
						action="file/upload"
						:limit="1"
						:type="2"
						:multiple="true"
						:disabled="ro.zizhizhengshu"
						:fileUrls="ruleForm.zizhizhengshu?ruleForm.zizhizhengshu:''"
						@change="zizhizhengshuUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item v-else-if="ruleForm.zizhizhengshu" label="资质证书" prop="zizhizhengshu" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.zizhizhengshu)">
				...
				<el-form-item v-else class="input" label="服务范围" prop="fuwufanwei" >
				...
				<el-form-item class="select" v-if="type!='info'"  label="服务类型" prop="fuwuleixing" >
					<el-select :disabled="ro.fuwuleixing" v-model="ruleForm.fuwuleixing" placeholder="请选择服务类型"  filterable>
						<el-option
...
<script>
	...
	export default {
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='fuwujineng'){
							this.ruleForm.fuwujineng = obj[o];
							this.ro.fuwujineng = true;
							continue;
						}
						if(o=='fuwuzhuangtai'){
						...
						if(o=='zizhizhengshu'){
							this.ruleForm.zizhizhengshu = obj[o];
							this.ro.zizhizhengshu = true;
							continue;
						}
						if(o=='fuwufanwei'){
					...
					this.ruleForm.fuwuzhuangtai = '空闲'; 					this.ruleForm.shouyi = Number('0'); 				}
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `fuwurenyuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fuwurenyuanCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
			...
			zizhizhengshuUploadChange(fileUrls) {
