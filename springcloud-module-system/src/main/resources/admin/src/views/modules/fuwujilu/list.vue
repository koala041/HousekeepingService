  <template>
	<div class="main-content" :style='{"width":"100%","padding":"20px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"0","boxShadow":"none","margin":"0 10px 5px 0","borderRadius":"0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","position":"relative"}' >
					...
					<div  :style='{"border":"1px solid #ddd","width":"auto","padding":"0 10px","margin":"0 5px 0px 0","borderRadius":"4px","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">员工姓名</label>
					...
					<div  :style='{"border":"1px solid #ddd","width":"auto","padding":"0 10px","margin":"0 5px 0px 0","borderRadius":"4px","display":"flex"}' class="select" label="服务确认" prop="fuwuqueren">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">服务确认</label>
						<el-select clearable v-model="searchForm.fuwuqueren" placeholder="请选择服务确认" >
							<el-option v-for="(item,index) in fuwuquerenOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('fuwujilu','确认')" @click="fuwuquerenCrossAddOrUpdateHandler(scope.row,'cross','','','fuwuqueren','已确认','已确认,待确认'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								确认
							</el-button>
							<el-button class="edit" v-if=" isAuth('fuwujilu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
		...
		<fuwuqueren-cross-add-or-update v-if="fuwuquerenCrossAddOrUpdateFlag" :parent="this" ref="fuwuquerenCrossaddOrUpdate"></fuwuqueren-cross-add-or-update>
...
<script>
	...
	import fuwuquerenCrossAddOrUpdate from "../fuwuqueren/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				fuwuquerenCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			fuwuquerenCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fuwuquerenCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fuwujilu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fuwuquerenCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fuwuquerenCrossaddOrUpdate.init(row.id,type);
				});
			},
			init () {
				this.fuwuquerenOptions = "已确认,待确认".split(',')
			},
			search() {
				this.pageIndex = 1;
				this.getDataList();
			...
			getDataList() {
				...
				if(this.searchForm.fuwuqueren!='' && this.searchForm.fuwuqueren!=undefined){
					params['fuwuqueren'] = this.searchForm.fuwuqueren
				}
