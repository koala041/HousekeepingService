  <template>
	<div class="main-content" :style='{"width":"100%","padding":"20px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0","margin":"0 0 5px","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto"}'>
					<el-button class="add" v-if="isAuth('shourumingxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shourumingxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						批量删除
					</el-button>


					<download-excel v-if="isAuth('shourumingxi','导出')" class="export-excel-wrapper" :fetch="getAllList" :fields="json_fields" name="收入明细.xlsx">
						<!-- 导出excel -->
			...
			<div :style='{"width":"100%","padding":"10px 0px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
						...
						label="提现日期">
						<template slot-scope="scope">
							{{scope.row.tixianriqi}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shourumingxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('shourumingxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shourumingxi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('shourumingxi','月收益统计')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery1.yuangongxingming" placeholder="请选择" @change="chartDialog1" clearable>
						<el-option v-for="item in yuangongxingmingChartOptions1" :label="item" :value="item"></el-option>
					</el-select>
					<div id="shouyiChart1" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
			</div>
		</template>
...
//导出excel
		...
		methods: {
			statChartClick() {
				if(this.isAuth('shourumingxi','月收益统计')) {
			...
			chartDialogShow1() {
				...
				this.$http.get('option/shourumingxi/yuangongxingming',
				).then(rs=>{
					this.yuangongxingmingChartOptions1 = rs.data.data
				})
				this.chartDialog1()
			},
			// 统计接口1
			chartDialog1() {
				this.$nextTick(()=>{
					var shouyiChart1 = echarts.init(document.getElementById("shouyiChart1"),'macarons');
					let params = {}
					if(this.chartQuery1.yuangongxingming) {
						params.conditionColumn = 'yuangongxingming'
						params.conditionValue = this.chartQuery1.yuangongxingming
					}
					this.$http({
						url: "shourumingxi/value/tixianriqi/shouyi/月",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								// 统计图设置对了吗
								xAxis.push(res[i].tixianriqi);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].tixianriqi
								})
							}
							var option = {};
