...
<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0","margin":"0 0 5px","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto"}'>
					<el-button class="add" v-if="isAuth('fuwuqueren','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('fuwuqueren','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('fuwuqueren','服务评分统计')" type="success" @click="chartDialogShow1">
			...
			<div :style='{"width":"100%","padding":"10px 0px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column  :resizable='true' prop="fankuitupian" width="200" label="反馈图片">
						<template slot-scope="scope">
							<div v-if="scope.row.fankuitupian">
								<img v-if="scope.row.fankuitupian.substring(0,4)=='http'&&scope.row.fankuitupian.split(',w').length>1" :src="scope.row.fankuitupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.fankuitupian)">
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('fuwuqueren','审核')" prop="sfsh" label="回复">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('fuwuqueren','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('fuwuqueren','结算')" @click="caiwujiesuanCrossAddOrUpdateHandler(scope.row,'cross','','','jiesuanzhuangtai','已结算','已结算,结算中'.split(',')[0])" type="success">
							...
							<el-button class="del" v-if="isAuth('fuwuqueren','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<caiwujiesuan-cross-add-or-update v-if="caiwujiesuanCrossAddOrUpdateFlag" :parent="this" ref="caiwujiesuanCrossaddOrUpdate"></caiwujiesuan-cross-add-or-update>
...
<script>
	...
	import caiwujiesuanCrossAddOrUpdate from "../caiwujiesuan/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				caiwujiesuanCrossAddOrUpdateFlag: false,
...
//导出excel
		...
		methods: {
			...
			caiwujiesuanCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.caiwujiesuanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fuwuqueren');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.caiwujiesuanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.caiwujiesuanCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "fuwuqueren/value/yuangongxingming/pingfen",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			shDialog(row){
				...
				if(row){
					this.shForm = {
						...
						fuwufeiyong: row.fuwufeiyong,
						wanchengshijian: row.wanchengshijian,
						fankuitupian: row.fankuitupian,
						pingfen: row.pingfen,
						fuwupingjia: row.fuwupingjia,
						...
						jiesuanzhuangtai: row.jiesuanzhuangtai,
						shhf: row.shhf,
						id: row.id
					}
				}
