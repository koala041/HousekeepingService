...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="完成时间" prop="wanchengshijian" >
					<el-date-picker
						...
						:picker-options="wanchengshijianPickerOptions"
						placeholder="完成时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.wanchengshijian" label="完成时间" prop="wanchengshijian" >
					<el-input v-model="ruleForm.wanchengshijian" placeholder="完成时间" readonly></el-input>
				</el-form-item>
				<el-form-item class="upload" v-if="type!='info' && !ro.fankuitupian" label="反馈图片" prop="fankuitupian" >
					<file-upload
						tip="点击上传反馈图片"
						action="file/upload"
						:limit="3"
						:disabled="ro.fankuitupian"
						:multiple="true"
						:fileUrls="ruleForm.fankuitupian?ruleForm.fankuitupian:''"
						@change="fankuitupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item class="upload" v-else-if="ruleForm.fankuitupian" label="反馈图片" prop="fankuitupian" >
					<img v-if="ruleForm.fankuitupian.substring(0,4)=='http'&&ruleForm.fankuitupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.fankuitupian" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.fankuitupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.fankuitupian.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.fankuitupian.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="评分" prop="pingfen" >
					<el-select :disabled="ro.pingfen" v-model.number="ruleForm.pingfen" placeholder="请选择评分"  filterable>
						<el-option
				...
				<el-form-item v-else class="input" label="评分" prop="pingfen" >
				...
				<el-form-item class="select" v-if="type!='info'"  label="结算状态" prop="jiesuanzhuangtai" style="display:none">
					<el-select :disabled="ro.jiesuanzhuangtai" v-model="ruleForm.jiesuanzhuangtai" placeholder="请选择结算状态"  filterable>
						<el-option
							v-for="(item,index) in jiesuanzhuangtaiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="结算状态" prop="jiesuanzhuangtai" >
					<el-input v-model="ruleForm.jiesuanzhuangtai"
						placeholder="结算状态" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item class="textarea" v-if="type!='info'" label="服务评价" prop="fuwupingjia" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.fuwupingjia" label="服务评价" prop="fuwupingjia"  class="textBox">
				<span class="text">{{ruleForm.fuwupingjia}}</span>
			</el-form-item>
		...
		data() {
			...
			return {
				...
				ruleForm: {
					...
					yonghuzhanghao: '',
					yonghuxingming: '',
					jiesuanzhuangtai: '结算中',
					shhf: '',
				},
				pingfenOptions: [],
				jiesuanzhuangtaiOptions: [],

				wanchengshijianPickerOptions: {
					disabledDate(time) {
				...
				rules: {
					...
					fankuitupian: [
						{ required: true, message: '反馈图片不能为空', trigger: 'blur' },
					],
					pingfen: [
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='fankuitupian'){
							this.ruleForm.fankuitupian = obj[o];
						...
						if(o=='jiesuanzhuangtai'){
							this.ruleForm.jiesuanzhuangtai = obj[o];
							this.ro.jiesuanzhuangtai = true;
							continue;
						}
					}
					this.ruleForm.jiesuanzhuangtai = '结算中'; 				}
				...
				this.jiesuanzhuangtaiOptions = "已结算,结算中".split(',')
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							if(this.ruleForm.fankuitupian!=null) {
								this.ruleForm.fankuitupian = this.ruleForm.fankuitupian.replace(new RegExp(this.$base.url,"g"),"");
							}
							...
							await this.$http({
								url: `fuwuqueren/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fuwuquerenCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
			...
			fankuitupianUploadChange(fileUrls) {
				this.ruleForm.fankuitupian = fileUrls;
			},
		}
	};
</script>
