  <template>
	<div class="main-content" :style='{"width":"100%","padding":"20px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0","margin":"0 0 5px","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto"}'>
					...
					<el-button class="edit" v-if="isAuth('fuwuyuyue','修改')" :disabled="dataListSelections.length!==1" type="primary" @click="addOrUpdateHandler(dataListSelections[0].id)">
						<span class="icon iconfont icon-xiugai13" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						编辑
					</el-button>


					<download-excel v-if="isAuth('fuwuyuyue','导出')" class="export-excel-wrapper" :fetch="getAllList" :fields="json_fields" name="服务预约.xlsx">
						<!-- 导出excel -->
				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('fuwuyuyue','派单')" @click="fuwudingdanCrossAddOrUpdateHandler(scope.row,'cross','否','是','dingdanzhuangtai','已派单','已派单,已取消,预约中'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								派单
							</el-button>
							<el-button class="btn8" v-if="isAuth('fuwuyuyue','取消')" @click="quxiaoyuyueCrossAddOrUpdateHandler(scope.row,'cross','否','是','dingdanzhuangtai','已派单不可取消','已派单,已取消,预约中'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								取消
							</el-button>
							...
							<el-button v-if="isAuth('fuwuyuyue','地图')" class="btn8" type="success" @click="showClick(scope.row)">
		...
		<template v-if="statType">
			<div class="statShow2">
				<div class="echarts1" v-if="isAuth('fuwuyuyue','预约金额统计')">
					<el-date-picker
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery1.yuyueriqiArr"
						type="daterange"
						range-separator="至"
					...
					<div id="fuwufeiyongChart1" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
				<div class="echarts2" v-if="isAuth('fuwuyuyue','预约服务类型统计')">
					<el-date-picker
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery2.yuyueriqiArr"
						type="daterange"
						range-separator="至"
						value-format="yyyy-MM-dd"
						format="yyyy-MM-dd"
					...
					<div id="fuwuleixingChart2" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
			</div>
		</template>
	...
	export default {
		data() {
			return {
				...
				yuyueriqiChartOptions1: [],
				chartQuery1: {},
				chartVisiable1: false,
				yuyueriqiChartOptions2: [],
				chartQuery2: {},
...
//导出excel
				json_fields: {
					...
					"项目图片": "xiangmutupian",    //常规字段
					"服务类型": "fuwuleixing",    //常规字段
					"服务费用": "fuwufeiyong",    //常规字段
					"位置": "weizhi",    //常规字段
					"服务内容": "fuwuneirong",    //常规字段
					"预约日期": "yuyueriqi",    //常规字段
					"预约时段": "yuyueshiduan",    //常规字段
					"预约时间": "yuyueshijian",    //常规字段
					"订单状态": "dingdanzhuangtai",    //常规字段
		...
		methods: {
			statChartClick() {
				if(this.isAuth('fuwuyuyue','预约金额统计')) {
					this.chartDialogShow1()
				}
				if(this.isAuth('fuwuyuyue','预约服务类型统计')) {
				...
				this.$confirm('是否支付所选订单？').then(async _ => {
					...
					for(let i in arr){
						...
						await this.$http.post('updateColumn/fuwuxinxi/2',{
							...
							csuConditionColumnValue: arr[i].xiangmubianhao
						}).then(rs=>{
							if(rs.data.code!=0) {
								changeType = false
								errMsg = '添加失败'
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var fuwufeiyongChart1 = echarts.init(document.getElementById("fuwufeiyongChart1"),'macarons');
					let params = {}
					if(this.chartQuery1.yuyueriqiArr&&this.chartQuery1.yuyueriqiArr.length) {
						params.conditionColumn = 'yuyueriqi,yuyueriqi'
						params.conditionValue = this.chartQuery1.yuyueriqiArr.join(',')
					}
					...
					this.$http({
						url: "fuwuyuyue/value/xiangmumingcheng/fuwufeiyong",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							fuwufeiyongChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								fuwufeiyongChart1.resize();
							};
						}
					})
				})
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					let params = {}
					if(this.chartQuery2.yuyueriqiArr&&this.chartQuery2.yuyueriqiArr.length) {
						params.conditionColumn = 'yuyueriqi,yuyueriqi'
						params.conditionValue = this.chartQuery2.yuyueriqiArr.join(',')
					}
					if(params.conditionColumn) {
						params.conditionColumn += ';' + 'ispay'
						params.conditionValue += ';' + '已支付'
					...
					this.$http({
						url: "fuwuyuyue/group/fuwuleixing",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			init () {
				...
				let urldingdanzhuangtai = 'fuwuyuyue/remind/dingdanzhuangtai/3';
				urldingdanzhuangtai+=`?remindstart=预约中`,

				this.$http({
					url: urldingdanzhuangtai,
					method: "get",
				}).then(({ data }) => {
					if (data && data.code == 0 && data.count>0 ) {
						const h = this.$createElement;
						var name = data.data
						this.dingdanzhuangtaiRemind = data.data
						var arr = []
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `${data.count}条订单状态预警`),
								h('p', null, [
									h('div', { style: 'text-align: center;font-weight: bold;padding: 0 0 10px;font-size: 16px' }, `服务已预约`),
