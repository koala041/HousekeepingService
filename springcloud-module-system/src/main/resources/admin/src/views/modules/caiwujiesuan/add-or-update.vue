	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="佣金占比" prop="yongjinzhanbi" >
					<el-input-number v-model="ruleForm.yongjinzhanbi" placeholder="佣金占比" :disabled="ro.yongjinzhanbi"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="佣金占比" prop="yongjinzhanbi" >
					<el-input v-model="ruleForm.yongjinzhanbi" placeholder="佣金占比" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'" label="收益" prop="shouyi" >
					<el-input v-model="shouyi" placeholder="收益" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.shouyi" label="收益" prop="shouyi" >
				...
				<el-form-item class="date" v-if="type!='info'" label="结算日期" prop="jiesuanriqi" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.jiesuanriqi" label="结算日期" prop="jiesuanriqi" >
					<el-input v-model="ruleForm.jiesuanriqi" placeholder="结算日期" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="提现状态" prop="tixianzhuangtai" >
					<el-select :disabled="ro.tixianzhuangtai" v-model="ruleForm.tixianzhuangtai" placeholder="请选择提现状态"  filterable>
						<el-option
							v-for="(item,index) in tixianzhuangtaiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="提现状态" prop="tixianzhuangtai" >
					<el-input v-model="ruleForm.tixianzhuangtai"
						placeholder="提现状态" readonly></el-input>
				</el-form-item>
			</template>
	...
	export default {
		...
		computed: {
			...
			shouyi : {
				get: function () {
					let c = this.ruleForm
					let a = c.fuwufeiyong*c.yongjinzhanbi
					this.ruleForm.shouyi = a?Number(Number(a).toFixed(2)):0
		...
		created() {
			this.ruleForm.jiesuanriqi = this.getCurDateTime()
		},
		methods: {
			imgPreView(url){
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='yongjinzhanbi'){
							this.ruleForm.yongjinzhanbi = obj[o];
							this.ro.yongjinzhanbi = true;
							continue;
						}
						if(o=='shouyi'){
						...
						if(o=='tixianzhuangtai'){
							this.ruleForm.tixianzhuangtai = obj[o];
							this.ro.tixianzhuangtai = true;
							continue;
						}
					}
					this.ruleForm.tixianzhuangtai = '已结算'; 				}
				...
				this.tixianzhuangtaiOptions = "已提现,已结算".split(',')
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							if(this.ruleForm.shouyi==0){
								this.$message.error('收益不能为空')
								return false
							}
							...
							await this.$http({
								url: `caiwujiesuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.caiwujiesuanCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
