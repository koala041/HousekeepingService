  <template>
	<div class="main-content" :style='{"width":"100%","padding":"20px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"0","boxShadow":"none","margin":"0 10px 5px 0","borderRadius":"0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","position":"relative"}' >
					...
					<div  :style='{"border":"1px solid #ddd","width":"auto","padding":"0 10px","margin":"0 5px 0px 0","borderRadius":"4px","display":"flex"}' class="select" label="服务类型" prop="fuwuleixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">服务类型</label>
					...
					<div  :style='{"border":"1px solid #ddd","width":"auto","padding":"0 10px","margin":"0 5px 0px 0","borderRadius":"4px","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">营业时间</label>
						<el-input v-model="searchForm.yingyeshijian" placeholder="营业时间" @keydown.enter.native="search()" clearable></el-input>
					</div>
		...
		<template v-if="statType">
			<div class="statShow4">
				<div class="echarts1" v-if="isAuth('fuwuxinxi','服务类型占比')">
					<div id="fuwuleixingChart1" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts2" v-if="isAuth('fuwuxinxi','服务项目收藏量')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery2.fuwuleixing" placeholder="请选择" @change="chartDialog2" clearable>
						<el-option v-for="item in fuwuleixingChartOptions2" :label="item" :value="item"></el-option>
					</el-select>
					<div id="storeupnumChart2" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
				<div class="echarts3" v-if="isAuth('fuwuxinxi','服务项目评分值统计')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery3.fuwuleixing" placeholder="请选择" @change="chartDialog3" clearable>
						<el-option v-for="item in fuwuleixingChartOptions3" :label="item" :value="item"></el-option>
					</el-select>
					<div id="totalscoreChart3" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
				<div class="echarts4" v-if="isAuth('fuwuxinxi','服务项目点赞量统计')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery4.fuwuleixing" placeholder="请选择" @change="chartDialog4" clearable>
						<el-option v-for="item in fuwuleixingChartOptions4" :label="item" :value="item"></el-option>
					</el-select>
					<div id="thumbsupnumChart4" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
			</div>
		</template>
	...
	export default {
		data() {
			return {
				...
				fuwuleixingChartOptions2: [],
				chartQuery2: {},
				chartVisiable2: false,
				fuwuleixingChartOptions3: [],
				chartQuery3: {},
				chartVisiable3: false,
				fuwuleixingChartOptions4: [],
				chartQuery4: {},
		...
		methods: {
			statChartClick() {
				if(this.isAuth('fuwuxinxi','服务类型占比')) {
					this.chartDialogShow1()
				}
				if(this.isAuth('fuwuxinxi','服务项目收藏量')) {
					this.chartDialogShow2()
				}
				if(this.isAuth('fuwuxinxi','服务项目评分值统计')) {
					this.chartDialogShow3()
				}
				if(this.isAuth('fuwuxinxi','服务项目点赞量统计')) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "fuwuxinxi/group/fuwuleixing",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			chartDialogShow2() {
				...
				this.$http.get('option/fuwuxinxi/fuwuleixing',
				).then(rs=>{
					this.fuwuleixingChartOptions2 = rs.data.data
				})
				this.chartDialog2()
			},
			// 统计接口2
			chartDialog2() {
				this.$nextTick(()=>{
					...
					let params = {}
					if(this.chartQuery2.fuwuleixing) {
						params.conditionColumn = 'fuwuleixing'
						params.conditionValue = this.chartQuery2.fuwuleixing
					}
					this.$http({
						url: "fuwuxinxi/value/xiangmumingcheng/storeupnum",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			chartDialogShow3() {
				...
				this.$http.get('option/fuwuxinxi/fuwuleixing',
					...
					this.fuwuleixingChartOptions3 = rs.data.data
				})
				this.chartDialog3()
			},
			// 统计接口3
			chartDialog3() {
				this.$nextTick(()=>{
					...
					let params = {}
					if(this.chartQuery3.fuwuleixing) {
						params.conditionColumn = 'fuwuleixing'
						params.conditionValue = this.chartQuery3.fuwuleixing
					}
					this.$http({
						url: "fuwuxinxi/value/xiangmumingcheng/totalscore",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			chartDialogShow4() {
				...
				this.$http.get('option/fuwuxinxi/fuwuleixing',
					...
					this.fuwuleixingChartOptions4 = rs.data.data
				})
				this.chartDialog4()
			},
			// 统计接口4
			chartDialog4() {
				this.$nextTick(()=>{
					...
					let params = {}
					if(this.chartQuery4.fuwuleixing) {
						params.conditionColumn = 'fuwuleixing'
						params.conditionValue = this.chartQuery4.fuwuleixing
					}
					this.$http({
						url: "fuwuxinxi/value/xiangmumingcheng/thumbsupnum",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			getDataList() {
				...
				if(this.searchForm.yingyeshijian!='' && this.searchForm.yingyeshijian!=undefined){
